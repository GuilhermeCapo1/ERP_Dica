  ...
  try {
    const projetos = await prisma.projeto.findMany({
      ...
      include: {
        cliente: {
          select: {
            id: true, nomeEmpresa: true, nomeFantasia: true, cnpj: true, cpf: true,
            email: true, telefone: true, endereco: true, cidade: true, estado: true,
          }
        },
        memorial: { select: { id: true, titulo: true, criadoEm: true }, take: 1 },
        orcamento: { select: { id: true, versao: true, valor: true, criadoEm: true }, orderBy: { versao: 'desc' }, take: 1 },
        contrato: { select: { id: true, numero: true, criadoEm: true }, take: 1 }
      },
      orderBy: { criadoEm: 'desc' }
    });
    res.json(projetos);
